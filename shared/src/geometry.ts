...
import type { Rect, Vec2, Zone } from "./types";
...
export function resolveMove(
  ...
  if (!hitsAnyWall(cx, cy, radius, walls)) return { x: cx, y: cy };

  let pos: Vec2 = { x: from.x, y: from.y };
  pos = furthestFree(pos, { x: cx, y: pos.y }, radius, walls);
  pos = furthestFree(pos, { x: pos.x, y: cy }, radius, walls);
  return pos;
}
...
/**
 * Audio gain from one listener to one speaker.
 *
 * Phase 2 wires this into a Web Audio GainNode. It lives here now because the
 * rule is a property of the world, not of the audio stack, and the renderer
 * uses it to draw the earshot circle.
 *
 * Order matters: zone membership is checked before distance, because a sealed
 * room overrides proximity in both directions.
 */
export function audioGain(
  listener: { x: number; y: number; zoneId: string | null },
  speaker: { x: number; y: number; zoneId: string | null },
  earshot: number,
): number {
